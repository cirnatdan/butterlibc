...
public import posix.sys.types; // for ssize_t, uid_t, gid_t, off_t, pid_t, useconds_t
...
extern(C) __gshared char*   optarg;
extern(C) __gshared int     optind;
extern(C) __gshared int     opterr;
extern(C) __gshared int     optopt;
extern(C) __gshared int __optpos;
extern(C) __gshared int __optreset=0;

alias __optpos optpos;
...
{
    FILE *f = stderr;
    //musl has a dummy so not going to implement this function atm
    //b = __lctrans_cur(b);
    //don't know how to lock yet
    //FLOCK(f);
    fputs(a, f)>=0
 ...
 * from musl
 */
int getopt(int argc, const scope char** argv, scope char* optstring)
{
    int i;
    wchar_t c, d;
    int k, l;
    ...
    optchar = cast(char*)argv[optind]+optpos;
    optpos += k;
    ...
    if (optstring[i] == ':') {
        ...
        if (optstring[i+1] != ':' || optpos) {
            optarg = cast(char*)argv[optind++] + optpos;
            optpos = 0;
        }
        if (optind > argc) {
            optopt = c;
...
version (CRuntime_Glibc)
{
    ...
    void       sync() @trusted;
    //int        truncate(const scope char*, off_t);
    uint       ualarm(uint, uint) @trusted;
    int        usleep(uint) @trusted;
    pid_t      vfork();
